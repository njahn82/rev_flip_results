...
```{r, fig.cap="Inclusion of reverse-flip journals in major bibliographic databases"}
...
```{r, fig.cap="Histogram of the average article volume per year. The red dashed line represents the median and the yellow dashed line shows the mean value of the distribution"}
article_volume %>%
  ...
  theme_minimal() +
  labs(x = "Average Journal Volume", y = "Number of Journals") +
    theme_minimal(base_family="Roboto") +
...
society_df %>%
  ...
  mutate(publisher = factor(publisher, levels = positions)) %>%
  ggplot(aes(journal_location, ..count.., , fill = factor(relation, 
                                     c("Society", "Institutional", "No relation")))) +
  geom_bar(position = position_stack(reverse = TRUE)) +
  coord_flip() +
  facet_wrap(~publisher, scales = "free_y", ncol = 3) +
  theme_minimal() +
  labs(x = "Journal Location", y = "Number of Reverse-Flip Journals") +
  scale_fill_manual("Affiliation",
                    values = c(
                      "Society" = "#23445D",
                      "Institutional" = "#AE4132",
  ...
  theme(panel.border=element_blank()) +
  theme(legend.position = "top",
          legend.justification = "right") -> p
ggsave("country_plot.png", p, width = 10, height = 10, dpi = 300)
```
...
```{r, fig.cap="Number of journals that converted from fully OA to a subscription-based business model per year, grouped by journal age at the time of the reverse flip"}
my_df %>%
  mutate(age_at_time_of_flip = year_reverse_flipped - year_founded) %>%
...
```{r, fig.cap='Developement of publication volume two years before and after the reverse flip.'}
tmp %>% 
  filter(lag %in% c(-2:2)) %>% 
  group_by(issn) %>% 
  filter(n() > 4) %>% 
...
```{r}
apcs <- readxl::read_xlsx("data/master_list.xlsx", sheet = 3) %>%
  filter(lisa_cases == 1) %>%
  mutate(
  ...
  filter(!is.na(diff)) %>%
  select(journal_name, diff, `Before Flip`, `After Flip`, `Current APC`) %>%
...
We were able to obtain publication fee list prices before and after the flip including the most recent level in 2018 for `r length(unique(apcs$journal_name))` journals. Figure compares them with each other, highlighting an considerable increase after the reverse flip.


```{r, fig.cap="APC list prices before and after the flip including most current level among reverse-flip journals. Please not that also "}
apcs %>% 
  ggplot(aes(apc_type, apc, fill = apc_type)) +
  geom_boxplot() +
  scale_x_discrete(limits = c("Before Flip", "After Flip", "Current APC")) +
  scale_fill_manual(values = c("Before Flip" = "#009392", 
                               "Current APC" = "#cf597e", 
                               "After Flip" = "#e9e29c")) +
  theme_minimal() +
  labs(x = "Timeline", y = "List Price", title = "APC Pricing") +
  # remove legend
  guides(fill=FALSE) +
  theme_minimal(base_family="Roboto") +
  theme(plot.margin=margin(30,30,30,30)) +
