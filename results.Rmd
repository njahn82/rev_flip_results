...
```{r fig_6, fig.cap="Reverse-flip journals affiliated with scholarly societies and research institutions per publisher"}
my_df %>%
    mutate(publisher = ifelse(publisher %in% "Brill Academic Publishers", "Brill", publisher)) %>%
...
```{r fig_7, fig.width=6, fig.height=12}
society_df %>%
...
```{r fig_8, fig.cap="Number of journals that converted from fully OA to a subscription-based business model per year, grouped by journal age at the time of the reverse flip"}
my_df %>%
  mutate(age_at_time_of_flip = year_reverse_flipped - year_founded) %>%
...
```{r fig_9, fig.cap='Discipline by journal age at the time of reverse flip, shown as proportion of the total number of reverse flip journals in the dataset. The colored areas represent the distribution of journals per Discipline.'}
...
```{r fig_10, fig.cap='Developement of publication volume two years before and after the reverse flip.'}
tmp %>% 
  filter(lag %in% c(-2:2)) %>% 
  group_by(issn) %>% 
  filter(n() > 4) %>% 
...
```{r, fig.cap="Scatterplot of APC list prices in USD based on pricing information for the year before the reverse-flip and for journals offering an hybrid open access option at the second half of 2018. Fully OA journals denoted with 0 did not charge an APC."}

ggplot(society_df, 
       aes(as.numeric(apc_pre), as.numeric(apc_now))) + 
  geom_point(aes(color = factor(relation, 
                                     c("Society", "Institutional", "No relation"))), alpha = 0.7) +
  coord_equal(ratio=1) +
  scale_color_manual("Affiliation",
                    values = c(
                      ...
                      "Institutional" = "#AE4132",
                      "No relation" = "grey60")) +
  labs(x = "Fully OA APC", y = "Hybrid OA APC") +
