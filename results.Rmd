...
country_tt_positions <- levels(tt$country)
ggplot(tt, aes(country, ..count.., fill = factor(relation, 
                                     c("Society", "Institutional", "No relation")))) +
  geom_bar(position = position_stack(reverse = TRUE)) +
...
p
ggsave("country_plot.png", p, width = 8, height = 6, dpi = 300)
```
...
ggplot(my_df_tt, aes(as.factor(year_reverse_flipped), 
  ...
  geom_histogram(stat = "count") +
  scale_fill_manual("Journal Age at Reverse Flip (in Years)", values = c("#fcde9c","#e34f6f","#7c1d6f")) +
  theme_minimal() +
  labs(x = "Year of Reverse Flip", y = "Number of Journals") +
    theme_minimal(base_family="Roboto") +
  theme(plot.margin=margin(30,30,30,30)) +
