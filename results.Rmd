...
#  group_by(issn) %>%
  summarise(Mean = mean(age_at_time_of_flip),
            ...
            `Q3` = quantile(age_at_time_of_flip, 3/4)) %>%
 gather(key = " ", value = "Journal Age") -> age_df
```

The dataset also includes information about the launch year of the journal, suggesting that not all journals in our sample were born online journals. In fact, around `r round(filter(my_df, !is.na(year_founded), year_founded < 1990) %>% nrow() / nrow(my_df) * 100)`% (N = `r filter(my_df, !is.na(year_reverse_flipped), year_founded < 1990) %>% nrow()`) of all journals found started before 1990. 

Table provides summary statistics of the journal age when the reverse flipped occured. Large age differences can be observed, ranging from one year of existences until `r age_df 
```

Figure presents the number of journals by journal a

Discipline by journal age at the time of reverse flip, shown as proportion of the total number of reverse flip journals in the dataset. The colored areas represent the distribution of journals per Discipline.
...
The dataset allows to analyse the extent of reverse flips across publishers. For this aim, publisher names for every journal were obtained using Crossref, and if not available, added manually. In case of a change of publishing house (To Do, describe the dynamics in methods section and, if possible, quantify it), the current publisher name was used. In total, `r n_distinct(my_df_cr$publisher)` different publishing houses with reversely flipped journals are included in our dataset. 
